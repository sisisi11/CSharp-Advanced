...
namespace _14.LegnedaryFarming
{
    using System;
    ...
    public class LegendaryFarming
    {
        ...
        public static void Main()
        {
            ...
            for (int i = 0; i < numberOfDragons; i++)
            {
                ...
                var dragonDamage = dragon[2].Equals("null") ? DefaultDamage
                    : int.Parse(dragon[2]);
                var dragonHealth = dragon[3].Equals("null") ? DefaultHealth
                    : int.Parse(dragon[3]);
                var dragonArmor = dragon[4].Equals("null") ? DefaultArmor
                    : int.Parse(dragon[4]);

                if (allDragons.ContainsKey(dragonType))
                {
                    allDragons[dragonType][dragonName] = new int[]
                        {dragonDamage, dragonHealth, dragonArmor};
                }
                else
                {
                    allDragons[dragonType] = new SortedDictionary<string, int[]>() {
                    {
                        dragonName, new int[]
                        {
                            dragonDamage, dragonHealth, dragonArmor
